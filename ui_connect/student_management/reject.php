...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  return $theValue;
		...
		if (isset($_GET['totalRows_studentSet_rej'])) {
		  $totalRows_studentSet_rej = $_GET['totalRows_studentSet_rej'];
		} else {
		  $all_studentSet = mysqli_query(dbconnect(), $query_studentSet);
		  $totalRows_studentSet_rej = mysqli_num_rows($all_studentSet);
		}
		$totalPages_studentSet = ceil($totalRows_studentSet_rej/$maxRows_studentSet)-1;
		...
		if (!empty($_SERVER['QUERY_STRING'])) {
		  ...
		  foreach ($params as $param) {
			if (stristr($param, "pageNum_studentSet") == false && 
				stristr($param, "totalRows_studentSet_rej") == false) {
		...
		$queryString_studentSet = sprintf("&totalRows_studentSet_rej=%d%s", $totalRows_studentSet_rej, $queryString_studentSet);
?>
