...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  return $theValue;
		...
		if (isset($_GET['totalRows_studentSet_trainee'])) {
		  $totalRows_studentSet_trainee = $_GET['totalRows_studentSet_trainee'];
		} else {
		  $all_studentSet = mysqli_query(dbconnect(), $query_studentSet);
		  $totalRows_studentSet_trainee = mysqli_num_rows($all_studentSet);
		}
		$totalPages_studentSet = ceil($totalRows_studentSet_trainee/$maxRows_studentSet)-1;
		...
		if (!empty($_SERVER['QUERY_STRING'])) {
		  ...
		  foreach ($params as $param) {
			if (stristr($param, "pageNum_studentSet") == false && 
				stristr($param, "totalRows_studentSet_trainee") == false) {
		...
		$queryString_studentSet = sprintf("&totalRows_studentSet_trainee=%d%s", $totalRows_studentSet_trainee, $queryString_studentSet);
?>
...
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">
        <div class="w3-container w3-card-2 w3-white w3-round w3-margin" id="onProcess">
                    ...
                    <input class="w3-input w3-border w3-padding" type="text" placeholder="Search for names.." id="traineeInput" onkeyup="traineeFn()">
                    
                    <table class="w3-table-all w3-margin-top w3-hoverable" id="traineeTable">
                        <tr>
                        ...
                        <?php do { ?>
                            <tr>
                              ...
                              <td><a href="portfolio.php?s_id=<?php echo $row_studentSet['s_id']; ?>"><?php echo $row_studentSet['s_lname']; ?></a></td>
    ...
    <script>
		function traineeFn() {
		  ...
		  input = document.getElementById("traineeInput");
