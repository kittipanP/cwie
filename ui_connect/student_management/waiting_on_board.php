...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  return $theValue;
		...
		mysqli_select_db($MyConnect, $database_MyConnect);
			...
			WHERE (student_status.status_id = '2')
		...
		$studentSet = mysqli_query(dbconnect(), $query_limit_studentSet) or die(mysqli_error());
		$row_studentSet = mysqli_fetch_assoc($studentSet);
		
		if (isset($_GET['totalRows_studentSet_wob'])) {
		  $totalRows_studentSet_wob = $_GET['totalRows_studentSet_wob'];
		} else {
		  $all_studentSet = mysqli_query(dbconnect(), $query_studentSet);
		  $totalRows_studentSet_wob = mysqli_num_rows($all_studentSet);
		}
		$totalPages_studentSet = ceil($totalRows_studentSet_wob/$maxRows_studentSet)-1;
		...
		if (!empty($_SERVER['QUERY_STRING'])) {
		  ...
		  foreach ($params as $param) {
			if (stristr($param, "pageNum_studentSet") == false && 
				stristr($param, "totalRows_studentSet_wob") == false) {
		...
		$queryString_studentSet = sprintf("&totalRows_studentSet_wob=%d%s", $totalRows_studentSet_wob, $queryString_studentSet);
?>
...
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">
        <div class="w3-container w3-card-2 w3-white w3-round w3-margin" id="onProcess">
                      <h2>Waiting on Board Student Lists</h2>
                      <p>Search for a name in the input field.</p>
                    
                    <input class="w3-input w3-border w3-padding" type="text" placeholder="Search for names.." id="waitInput" onkeyup="waitFn()">
                    
                    <table class="w3-table-all w3-margin-top w3-hoverable" id="waitTable">
                        <tr>
    ...
    <script>
		function waitFn() {
		  ...
		  input = document.getElementById("waitInput");
		  filter = input.value.toUpperCase();
		  table = document.getElementById("waitTable");
